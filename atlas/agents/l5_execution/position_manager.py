...
class PositionManager:
    ...
    async def update_mark_to_market(self):
        ...
        async with self.db.engine.begin() as conn:
            ...
            r = await conn.execute(text("SELECT id, strategy_id, symbol, side, qty, avg_price, trace_id, feature_snapshot_id FROM positions"))
            ...
            r_prices = await conn.execute(
                text("SELECT DISTINCT ON (symbol) symbol, close FROM market_data_l1 WHERE symbol = ANY(:symbols) ORDER BY symbol, time DESC"),
                {"symbols": symbols}
            )
            ...
            for pos in positions:
                ...
                await conn.execute(
                    text("UPDATE positions SET unrealized_pnl = :pnl, last_mark_time = NOW() WHERE id = :id"),
                ...
                if pct_move <= -0.05 or pct_move >= 0.10:
                    ...
                    logger.info(f"Position {sym} ({sid}) triggered {exit_reason} at {current_price}")
                    # In a fully connected system, we'd emit an exit signal here.
                    # For demo purposes, we will close it directly.
                    await self._execute_exit(conn, sid, sym, side, qty, current_price, unrealized_pnl, exit_reason, trace_id=tid, feature_snapshot_id=fsid)

    async def _execute_exit(self, conn, strategy_id, symbol, side, qty, exit_price, realized_pnl, reason, trace_id=None, feature_snapshot_id=None):
    ...
    async def active_position_exists(self, strategy_id: str, symbol: str) -> bool:
        ...
        query = "SELECT 1 FROM positions WHERE strategy_id = :sid AND symbol = :sym LIMIT 1"
        async with self.db.engine.connect() as conn:
            res = await conn.execute(text(query), {"sid": strategy_id, "sym": symbol})
            return res.fetchone() is not None

    async def open_position(self, strategy_id: str, symbol: str, side: str, qty: float, avg_price: float, broker_name: str, trace_id=None, feature_snapshot_id=None):
        ...
        async with self.db.engine.begin() as conn:
            ...
            if existing:
                ...
                await conn.execute(text(update_query), {"qty": new_qty, "avg_price": new_avg, "id": existing[0], "tid": trace_id, "fsid": feature_snapshot_id})
            else:
                insert_query = """
                ...
                await conn.execute(text(insert_query), {
                    ...
                    "tid": trace_id, "fsid": feature_snapshot_id
                })
