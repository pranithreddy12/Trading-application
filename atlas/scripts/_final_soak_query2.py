"""Phase 26H — Final Soak Results Query (v2)."""
import asyncio, re
...
async def check():
    ...
    rows = await conn.fetch("SELECT source, COUNT(*) as cnt FROM scout_signals GROUP BY source ORDER BY cnt DESC")
    print(f'  ALL-TIME sources ({len(rows)}):')
    for r in rows:
        print(f'    {r[0]}: {r[1]}')
    ...
    print(f'  mutation_record total: {r[0]}')

    # ── Scout source tables ──
    print(f'\n  Scout source tables (this run):')
    ...
    print(f'\n  strategy_generation_log (this run): {r[0]}')

    # Check scout_mirror_debug_log
    try:
        r = await conn.fetchrow("SELECT COUNT(*) FROM scout_mirror_debug_log WHERE created_at > NOW() - INTERVAL '2 hours'")
        print(f'  scout_mirror_debug_log (this run): {r[0]}')
    except:
        pass

    # Check for newly generated strategies and their features
    ...
    """)
    if r:
        print(f'\n  New strategy: id={r[0]}, created={str(r[1])[:19]}, status={r[2]}')
    else:
        print(f'\n  No new strategies generated this run')
