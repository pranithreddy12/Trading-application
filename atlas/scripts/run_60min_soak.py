...
LOG = Path(__file__).resolve().parents[1] / 'logs' / 'run_60min.log'
...
def main():
    ...
    with LOG.open('a', encoding='utf-8') as f:
        f.write(f"SOAK_STARTED: {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}\n")
        f.flush()
                ...
                if time.time() - start_time >= timeout:
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    f.write(f"SOAK_COMPLETE: {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}\n")
                    break
        except Exception as e:
            f.write(f"SOAK_ERROR: {e!r}\n")
            try:
                proc.kill()
            except Exception:
                pass
