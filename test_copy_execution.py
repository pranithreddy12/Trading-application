#!/usr/bin/env python
"""
Smoke Test Phase 2: Insert leader order and verify copy execution
"""
import asyncio
...
async def main():
        ...
        if logs:
            ...
            for i, log in enumerate(logs, 1):
                ...
                if log[6] == 'filled' and float(log[5]) == 5.0:
                    ...
                    print(f"    ℹ SKIPPED: {log[7]}")
                else:
                    print(f"    ℹ Status: {log[6]}")
        else:
            print("✗ No copy execution log entries found yet.")
            print("  → Copy trader may not have detected the leader order yet")
            print("  → Check if copy_trader is running with: python -m atlas.agents.l5_execution.copy_trader")
    ...
    async with db.engine.connect() as conn:
        ...
        print(f"Filled copies: {filled_count}")
